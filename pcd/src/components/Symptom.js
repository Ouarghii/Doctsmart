...
function Symptom() {
  ...
  function handleSymptomChange(event, index) {
    ...
    newSymptoms[index] = event.target.value === "" ? "*" : event.target.value;
    // check if the first three select options are selected before updating the state
  ...
  function handleSubmit(event) {
    ...
    console.log(`/predict/${symptomString}`);
    window.location.href = `http://localhost:5000/predict/${symptomString}`;
  }

  return (
    <div className="Symcontain">
      <div class="four">
        <h1><span>Disease Prediction </span> Select at least <em>Three</em> Symptoms</h1>
      </div>
      <form onSubmit={handleSubmit}>
        ...
        {selectedSymptoms.some(symptom => symptom !== "*") && (
  <div className="selectedsym">
    <div class="eight">
      <h1>Selected Symptoms</h1>
    </div>
    <ul>
      {selectedSymptoms.filter(symptom => symptom !== "*").map((symptom, index) => (
